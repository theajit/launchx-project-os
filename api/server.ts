...
app.post('/auth/bootstrap', async (request, reply) => {
  ...
  if (body.role !== 'CEO') return reply.forbidden('Workspace bootstrap must create the CEO account first');
  const existingOrg = await query<{ id: string }>('select id from organizations limit 1');
  if (existingOrg.length) return reply.conflict('Workspace already exists. Ask CEO to create team members.');
  ...
  await query('insert into organization_members (organization_id, user_id, role) values ($1,$2,$3)', [org[0].id, user[0].id, 'CEO']);
  const token = app.jwt.sign({ userId: user[0].id, organizationId: org[0].id, role: 'CEO', email: body.email });
  return { token, user: { id: user[0].id, email: body.email, name: body.name, role: 'CEO' }, organization: { id: org[0].id, name: body.organizationName } };
});
...
app.get('/team/members', async (request) => { const user = await requireAuth(request); return query('select u.id, u.email, u.name, om.role, om.created_at from users u join organization_members om on om.user_id=u.id where om.organization_id=$1 order by case om.role when \'CEO\' then 1 when \'CTO\' then 2 when \'CTPO\' then 3 when \'CGO\' then 4 else 9 end, u.name', [user.organizationId]); });
app.post('/team/members', async (request, reply) => {
  const user = await requireAuth(request); if (!requireCEO(user, reply, 'Only CEO can create CTO/CTPO/CGO accounts')) return;
  ...
  if (!memberId) { const passwordHash = await bcrypt.hash(body.password, 12); const created = await query<{ id: string }>('insert into users (email, name, password_hash) values ($1,$2,$3) returning id', [body.email, body.name, passwordHash]); memberId = created[0].id; }
...
app.patch('/team/members/:id', async (request, reply) => { const user = await requireAuth(request); if (!requireCEO(user, reply)) return; const params = z.object({ id: z.string().uuid() }).parse(request.params); const body = z.object({ role: z.enum(['CTO','CTPO','CGO']) }).parse(request.body); await query('update organization_members set role=$1 where organization_id=$2 and user_id=$3 and role <> \'CEO\'', [body.role, user.organizationId, params.id]); await logActivity(user, 'member', params.id, 'role_updated', { role: body.role }); return { ok: true }; });
